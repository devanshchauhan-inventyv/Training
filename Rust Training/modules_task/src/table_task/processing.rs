...
fn process_data(mut input_data: InputData) -> Table {
    ...
    let header_cell_height =
        spliting_heights_for_header(&mut input_data.headerRow.title, cell_content_width, 18, 20);
    let mut data_row_cell_height = looping_for_datarow_cells(
        len_of_dataRows,
        &mut input_data.dataRows.rows,
        cell_content_width,
    );
...
pub fn creating_rows(
    vec_of_cells: Vec<Vec<Cell>>,
    header_cell_height: Vec<usize>,
    data_row_cell_height: Vec<Vec<usize>>,
) -> Vec<Row> {
...
pub fn creating_cells(
    Vec_of_Cells: &mut Vec<Vec<Cell>>,
    data: InputData,
    cell_width: usize,
...
pub fn looping_for_datarow_cells(
    len_of_dataRows: usize,
...
pub fn spliting_heights_for_header(
    data: &mut Vec<String>,
    cell_content_width: usize,
    header_font_size: usize,
    mut cell_height: usize,
) -> Vec<usize> {
    ...
    for content in data {
        ...
        while index < content.len() {
            ...
            width +=
                *HASHMAP.get(&c).expect("Character is not in hashmap") * header_font_size as f64;
